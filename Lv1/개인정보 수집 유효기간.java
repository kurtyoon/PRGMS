 ...
 class Solution {
     public int[] solution(String today, String[] terms, String[] privacies) {
         ...
         for (int i = 0; i < privacies.length; i++) {
             ...
             if (getDate(privacy[0]) + map.get(privacy[1]) * 28 <= date) { // 수집 일자 + 유효기간의 일 수 <= 오늘 날짜의 일 수 -> 만료된 경우
                 answer.add(i + 1); // 현재 index로 0부터 시작하므로 +1해서 추가
             }
         }
     ...
     private int getDate(String today) {
         String date[] = today.split("\\."); // split은 정규식 형태로 값을 입력. .은 정규식에서는 모든 문자를 의미하므로 \\를 붙여줘야 함.
         int year = Integer.parseInt(date[0]);
         int month = Integer.parseInt(date[1]);
         int day = Integer.parseInt(date[2]);
         return (year * 12 * 28) + (month * 28) + day; // 년 월 일의 형태를 일 수로 전환
     }
 }

 /*
  당시 코테 칠 때 상당히 오래걸렸던 문제
  C++에는 split이 없어서 직접 구현해야 했지만 Java에는 split이 있어서 편하게 풀 수 있었음
  역시...
  아직 C++이랑 Java가 헷갈림
  Java를 깊게 공부할 필요 있음
  */